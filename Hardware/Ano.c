...
void Ano_Anl(uint8_t *Data_Pack)          /* 数据解析 */
{
    ...
    if(Data_Pack[2] == 0xE1) /* 参数读取ID 0XE1 */
    {
       uint16_t id = Data_Pack[4] + ((uint16_t)Data_Pack[5]<<8);
        ...
        {            
            case 1: 
                Ano_SentPar(id,Position_KP*1000); /* Kp放大1000倍发送 */
            break;
            case 2:
                Ano_SentPar(id,Position_KI*1000); /* Ki放大1000倍发送 */
            break;
            case 3:
                Ano_SentPar(id,Position_KD*1000); /* Kd放大1000倍发送 */
            break;
            case 4:
                Ano_SentPar(id,num);              /* 转速  */
            break;
    ...
    if(Data_Pack[2] == 0xE2) /* 参数写入ID 0XE2，接收到的是整数*/
    {
       uint16_t id = Data_Pack[4] + ((uint16_t)Data_Pack[5]<<8);
        ...
        {
            case 1:
                Position_KP=(float)*(int *)(&Data_Pack[6])/1000;
            break;
            case 2:
                Position_KI=(float)*(int *)(&Data_Pack[6])/1000;
            break;
            case 3:
                Position_KD=(float)*(int *)(&Data_Pack[6])/1000;
            break;
            case 4:
							  num=(float)*(int *)(&Data_Pack[6]); //除10还原一位小数
